...
- Typical run:
  ...
  3) Create a simplified MusicXML via ChatGPT:
    - In ChatGPT ("ChatGPT 5 Thinking" model recommended), set the System prompt to `src/piano_learning/resources/system_instructions_for_chatgpt.j2` (fill in `{{ BASENAME }}` and `{{ TIMESTAMP }}`)
    - Paste the User prompt from `src/piano_learning/resources/user_prompt_for_chatgpt.j2` (fill in `{{ BASENAME }}` and `{{ TIMESTAMP }}`)
    - Set BASENAME (e.g., `Your_Score`) and a timestamp like `YYYYMMDD-HHMMSS`
